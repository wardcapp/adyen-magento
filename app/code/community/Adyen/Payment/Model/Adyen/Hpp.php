...
//         $adyFields['pos.sig'] = base64_encode(pack('H*', $signPOS));
    ...
    public function getAvailableHPPTypes()
    {
        ...
        if ($results === false) {
            ...
            Mage::log(
                "Payment methods are not available on this merchantaccount\skin result is: " . curl_error($ch),
                self::DEBUG_LEVEL,
                'http-request.log',
                true
            );
            Mage::throwException(
                Mage::helper('adyen')->__('Payment methods are not available on this merchantaccount\skin')
            );
        } else {
            /**
            ...
            if ($results_json == null) {
                ...
                Mage::log(
                    "Payment methods are empty on this merchantaccount with the selected skin,hmac,amount,country check if these settings are correct results_json result is:"
                    . $results,
                    self::DEBUG_LEVEL,
                    'http-request.log',
                    true
