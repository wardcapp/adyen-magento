...
class Adyen_Payment_Block_Form_PosCloud extends Mage_Payment_Block_Form
{
    ...
    public function retrieveConnectedTerminals()
    {
        ...
        $cacheId = "adyen_terminals_" . $storeId;
        if (!Mage::helper('adyen')->getConfigDataDemoMode($storeId) && $cacheData = Mage::app()->getCache()->load($cacheId)) {
            $response = $cacheData;
        } else {
            $response = $api->retrieveConnectedTerminals($storeId);
            if (!empty(json_decode($response)->uniqueTerminalIds)) {
